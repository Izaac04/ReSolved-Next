...
def main():
    ...
    ap.add_argument(
        "--best-metric",
        type=str,
        default="sampled_reward_avg",
        help="Validation metric used to select the best GFlowNet checkpoint",
    )
    ...
    ap.add_argument(
        ...
        default="auto",
        help="Whether lower or higher values are better for the selected best-checkpoint metric",
    )
    args = ap.parse_args()
    ...
    cfg.log_dir = args.log_dir or os.path.join(BASE_DIR, "logs", f"resolve_my_fragments_{timestamp}")
    ...
    trainer = MyFragmentsResolveTrainer(
        ...
        best_checkpoint_mode=args.best_metric_mode,
    )
    trainer.run()
